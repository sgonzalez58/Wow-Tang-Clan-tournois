...
const UpdateParty = () =>{
    ...
    useEffect(()=>{
        ...
        .then((characters) => fetch(`http://localhost:3000/parties/${params.partyId}`, {
            method: 'GET'
        })
        .then((response) => {
            return response.json()
        ...
        .then((data) => {
            const rawParty = data.data;
            const damageMember1 = characters.find((character) => character.role == 'damage' && rawParty.members.split(',').includes(character.name)).id;
            const damageMember2 = characters.find((character) => character.role == 'damage' && rawParty.members.split(',').includes(character.name) && character.id != damageMember1).id;
            const damageMember3 = characters.find((character) => character.role == 'damage' && rawParty.members.split(',').includes(character.name) && character.id != damageMember1 && character.id != damageMember2).id;
            setFormData(() => ({
                partyName: rawParty.partyname,
                tankMember: characters.find((character) => character.role == 'tank' && rawParty.members.split(',').includes(character.name)).id,
                healerMember: characters.find((character) => character.role == 'healer' && rawParty.members.split(',').includes(character.name)).id,
    ...
    const handleDeleteParty = async () =>{
        await fetch(`http://localhost:3000/parties/delete/${params.partyId}`, {
            method: "GET"
        })
        .then((res) => {
            return res.json()
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if(e.nativeEvent.submitter.id == 'deleteParty'){
            if(confirm("Voulez-vous vraiment supprimer cette équipe ?")){
                return handleDeleteParty()
            }
        }
        ...
        fetch(`http://localhost:3000/parties/update/${params.partyId}`, {
            method: "POST",
            body: JSON.stringify({
    ...
    return (
        <div className="max-w-md mx-auto bg-white p-8 rounded-lg shadow-md mt-8">
            <h1 className="text-2xl font-bold text-center text-gray-800 mb-8">Modification d'équipe</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                    ...
                    <select id={`tankMember`} name="tankMember" onChange={handleChangeInput} value={formData.tankMember}
                ...
                <div className="space-y-2">
                    ...
                    <select id={`healerMember`} name="healerMember" onChange={handleChangeInput} value={formData.healerMember}
                    className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                <div className="space-y-2">
                    ...
                    <select id={`damageMember1`} name="damageMember1" onChange={handleDamageDealerInput} value={formData.damageMember1}
                    className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                <div className="space-y-2">
                    ...
                    <select id={`damageMember2`} name="damageMember2" onChange={handleDamageDealerInput} value={formData.damageMember2}
                    className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                <div className="space-y-2">
                    ...
                    <select id={`damageMember3`} name="damageMember3" onChange={handleDamageDealerInput} value={formData.damageMember3}
                    className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
